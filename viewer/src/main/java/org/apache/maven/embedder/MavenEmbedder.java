...
public class MavenEmbedder {
    ...
    private List<ArtifactRepository> activeProfileRepositories;
    ...
    public MavenProject readProject(File mavenProject)
        ...
        return readProject(mavenProject, true);
    }

    public MavenProject readProject(File mavenProject, boolean checkDistributionManagementStatus)
            throws ProjectBuildingException {
        return mavenProjectBuilder.build(mavenProject, localRepository, profileManager, checkDistributionManagementStatus);
    }
    ...
    public MavenProject buildFromRepository(Artifact artifact,
            ...
            throws ProjectBuildingException {
        if (isOffline()) {
            remoteArtifactRepositories.clear();
        } else {
            // Also use repositories defined in active profiles
            remoteArtifactRepositories.addAll(activeProfileRepositories);
        }
        return mavenProjectBuilder.buildFromRepository(
                artifact, remoteArtifactRepositories, localRepository, allowStubModel);
    }
    ...
    private void initActiveProfiles() {
        activeProfileRepositories = new ArrayList<ArtifactRepository>();
        Map<String, ArtifactRepository> reposMaps = new HashMap<String, ArtifactRepository>();
        String repoId;
        try {
            List<Profile> myActiveProfiles = profileManager.getActiveProfiles();
            for (Profile profile : myActiveProfiles) {
                Repository repo;
                for (Object o : profile.getRepositories()) {
                    repo = (Repository) o;
                    repoId = repo.getId();
                    try {
                        // Avoid duplicated repositories
                        if (!reposMaps.containsKey(repoId)) {
                            reposMaps.put(repoId, createRepository(repo.getUrl(), repoId));
                        }
                    } catch (ComponentLookupException e) {
                        logger.warn("Could not create repository: " + repoId, e);
                    }
                }
            }
        } catch (ProfileActivationException e) {
            logger.warn("Could not get active profiles list", e);
        }
        activeProfileRepositories.addAll(reposMaps.values());
    }
