...
public class JcrHelper implements DisposableBean {
        ...
        if (repo == null) {
            throw new RepositoryException(
                    "The repository '" + repoKey + "' is not configured." +
                            "Some files might have been incorrectly imported - Please remove " +
                            "them manually.");
        }
        if (!repo.handles(relPath)) {
            throw new RepositoryException(
                    "The repository '" + repoKey + "' rejected the artifact '" + relPath +
                            "' due to its snapshot/release handling policy. Some files might " +
                            "have been incorrectly imported - Please remove them manually.");
        }
        if (!repo.accepts(relPath)) {
            throw new RepositoryException(
                    "The repository '" + repoKey + "' rejected the artifact '" + relPath +
                            "' due to its include/exclude patterns settings. Some files might " +
                            "have been incorrectly imported - Please remove them manually.");
        }
    ...
    private static void cleanupAfterLastShutdown() {
        ...
        if (tmpDirPath != null) {
            ...
            String[] wildcards = {"bin*.tmp", "maven-artifact*", "tmp"};
            Collection<File> tmpfiles =
                    FileUtils.listFiles(tmpDir, new WildcardFilter(wildcards), null);
