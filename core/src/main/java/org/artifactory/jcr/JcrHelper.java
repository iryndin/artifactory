 ...
 */
public class JcrHelper implements Startable {
    @SuppressWarnings({"UNUSED_SYMBOL", "UnusedDeclaration"})
    private final static Logger LOGGER = Logger.getLogger(JcrHelper.class);

    private JackrabbitRepository repository;

    public JcrHelper(CentralConfig cc) {
        try {
            String repoHome = cc.getLocalRepositoriesDir() + "/jcr";
    ...
    public JcrSessionWrapper bindSession() {
        ...
        JcrSessionWrapper existingSession = JcrSessionHolder.getSession();
        if (existingSession != null) {
            return existingSession;
        } else {
            JcrSessionWrapper wrapper = newSession();
            JcrSessionHolder.setSession(wrapper);
            return wrapper;
        }
    }

    public JcrSessionWrapper unbindSession(boolean save) {
        JcrSessionWrapper wrapper = JcrSessionHolder.getSession();
        try {
            closeSession(wrapper, save);
        } catch (RepositoryException e) {
            LOGGER.warn("Failed to close jcr session.", e);
        } finally {
            JcrSessionHolder.reset();
        }
        return wrapper;
    }
     ...
     */
    public static void importFile(Node repoNode, Node parentNode, File file)
            throws RepositoryException, IOException {
        BufferedInputStream bis = new BufferedInputStream(new FileInputStream(file));
        importStream(parentNode, file.getName(), repoNode.getName(), file.lastModified(), bis);
    }
    ...
    public static void importFolder(
            Node repoNode, Node parentNode, File directory) throws RepositoryException,
            IOException {
        String repoKey = repoNode.getName();
        File[] dirEntries = directory.listFiles();
        for (File direntry : dirEntries) {
            String fileName = direntry.getName();
            if (direntry.isDirectory()) {
                Node childNode;
                childNode = getOrCreateChildNode(parentNode, fileName, repoKey);
                LOGGER.info("Importing directory '" + fileName + "' into '" + repoKey + "'...");
                importFolder(repoNode, childNode, direntry);
            } else {
                LOGGER.info("Importing file '" + fileName + "' into '" + repoKey + "'...");
                importFile(repoNode, parentNode, direntry);
            }
        }
    }
    ...
    private static Node getOrCreateChildNode(Node parentNode, String folderName, String repoKey)
            throws RepositoryException {
        Node childNode;
