...
public abstract class ArchiveIndexer {
    ...
    //Don't change these property names without updating the indexing config!

    public static final String PROP_ARTIFACTORY_ARCHIVE_ENTRY = "artifactory:archiveEntry";
    public static final String PROP_ARTIFACTORY_ARCHIVE_INDEXED = "artifactory:archiveIndexed";
    ...
    public static void index(JcrFile file) {
        ...
        try {
            ...
            node.setProperty(PROP_ARTIFACTORY_ARCHIVE_ENTRY, sb.toString(), PropertyType.STRING);
            //Mark the files as indexed
            node.setProperty(PROP_ARTIFACTORY_ARCHIVE_INDEXED, true);
            node.save();
    ...
    public static boolean markArchiveForIndexing(Node archiveNode, boolean force) throws RepositoryException {
        ...
        if (nodeType.equals(JcrFile.NT_ARTIFACTORY_FILE)) {
            if (force || !archiveNode.hasProperty(PROP_ARTIFACTORY_ARCHIVE_INDEXED)) {
                archiveNode.setProperty(PROP_ARTIFACTORY_ARCHIVE_INDEXED, false);
    ...
    public static void indexMarkedArchives() {
        ...
        try {
            ...
            String queryStr = "/jcr:root" + JcrPath.get().getRepoJcrRootPath() + "//element(*, " +
                    JcrFile.NT_ARTIFACTORY_FILE + ")[@" + ArchiveIndexer.PROP_ARTIFACTORY_ARCHIVE_INDEXED +
                    "= false()]";
            ...
            NodeIterator nodes = queyResult.getNodes();
            while (nodes.hasNext()) {
