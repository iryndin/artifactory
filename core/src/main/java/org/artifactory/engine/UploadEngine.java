...
public class UploadEngine {
    ...
    public UploadEngine(CentralConfig cc) {
        this.cc = cc;
    }
    ...
    public void process(ArtifactoryRequest request, ArtifactoryResponse response)
        ...
        String repoKey = request.getTargetLocalRepoKey();
        //Sanity check
        if (repoKey == null) {
        ...
        if (ar.isValid()) {
            ...
            try {
                ...
                Artifact artifact = mavenWrapper.createArtifact(ar);
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Deploying '" + fileName + "' as '" + artifact + "'.");
                }
                mavenWrapper.deploy(file, artifact, repo);
            } finally {
                file.delete();
            }
        }
