...
public class UploadEngine {
    ...
    public void process(ArtifactoryRequest request, ArtifactoryResponse response)
        ...
        /*
        //TODO: [by yl] Non-unique snapshots + their metadata cannot be easily controlled this way
        SimpleRepoResource res = new SimpleRepoResource(path, repo);
        InputStream is = null;
        try {
            res.setLastModifiedTime(System.currentTimeMillis());
            SecurityHelper security = context.getSecurity();
            RepoPath repoPath = new RepoPath(repo.getKey(), res.getPath());
            if (!security.canDeploy(repoPath)) {
                response.sendError(HttpStatus.SC_FORBIDDEN);
                return;
            }
