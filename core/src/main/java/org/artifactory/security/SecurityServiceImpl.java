...
public class SecurityServiceImpl implements InternalSecurityService {
    ...
    private void checkForExternalConfiguration() {
        ...
        if (configurationFile.isFile()) {
            ...
            try {
                ...
                throw new RuntimeException("security.import.xml file could not be found.", e);
            }
            XStream xStream = XStreamFactory.create(SecurityInfo.class);
            SecurityInfo descriptorToSave;
            try {
                descriptorToSave = ((SecurityInfo) xStream.fromXML(reader));
            } catch (Exception e) {
            ...
            org.artifactory.util.FileUtils.switchFiles(configurationFile, new File(etcDir, "security.bootstrap.xml"));
    ...
    public String currentUsername() {
        ...
        return (authentication != null ? authentication.getName() : USER_SYSTEM);
    }
    ...
    public UserInfo findOrCreateExternalAuthUser(String userName) {
        UserInfo userInfo;
        try {
            userInfo = findUser(userName);
        } catch (UsernameNotFoundException e) {
            ...
            userInfo = new UserInfoBuilder(userName).build();
            Set<String> defaultGroups = getNewUserDefaultGroupsNames();
