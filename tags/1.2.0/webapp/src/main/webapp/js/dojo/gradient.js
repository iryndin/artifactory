...
function createGradient() {
    ...
    for (i = 0; i < objArray.length; i++) {
        ...
        if (document.all && !window.opera) {
            ...
            "progid:DXImageTransform.Microsoft.Gradient(GradientType=" + gType +
            ",StartColorStr=\"#" + params[1] + "\",EndColorStr=\"#" + params[2] + "\")";
        } else {
            ...
            for (p = 0; p < colorArray.length; p++) {
                ...
                g.setAttribute("style", "position:absolute;z-index:0;top:" + y + "px;left:" + x +
                                        "px;height:" + h + "px;width:" + w +
                                        "px;background-color:rgb(" + colorArray[p][0] + "," +
                                        colorArray[p][1] + "," + colorArray[p][2] + ");");
...
function createColorPath(color1, color2) {
    ...
    do {
        ...
        setColorHue(longHexToDec(color1), colorPercent, longHexToDec(color2));
        colorPercent -= .01;
...
function setColorHue(originColor, opacityPercent, maskRGB) {
    ...
    for (w = 0; w < originColor.length; w++) returnColor[w] =
...
function makeGrandParent(obj) {
    ...
    disp == "block" ? nSpan = document.createElement("div") :
