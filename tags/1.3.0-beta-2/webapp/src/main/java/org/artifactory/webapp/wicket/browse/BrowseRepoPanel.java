...
public class BrowseRepoPanel extends TitlePanel {
    ...
    public BrowseRepoPanel(String string) {
        ...
        treeController.add(new AbstractStringResponseAjaxBehavior() {
            ...
            @SuppressWarnings({"UnnecessaryLocalVariable", "OverlyComplexMethod"})
            protected String getResponse() {
                ...
                if ("download".equals(action)) {
                    ...
                    for (JcrFsItem item : items) {
                        ...
                        RepoPath childRepoPath = new RepoPath(item.getRepoKey(), relPath);
                        ...
                        final boolean childDeployer = security.canDeploy(childRepoPath);
                        //Skip checksum and index files
                        if (MavenUtils.isChecksum(relPath) || MavenUtils.isHidden(relPath)) {
                            continue;
                        }
                        ...
                        String id =
                                getRepoRootPrefix(repo) + Strings.toEscapedUnicode(
        ...
        ListView trees = new ListView("repositories", repos) {
            protected void populateItem(ListItem item) {
                ...
                boolean deployer = security.canDeploy(SecuredRepoPath.forRepo(key));
        ...
        contentViewer.add(new AbstractDefaultAjaxBehavior() {
            ...
            protected void respond(AjaxRequestTarget target) {
                ...
                if (content != null && !item.isDirectory()) {
                    InputStream is = null;
                    try {
        ...
        contentViewer.setOutputMarkupId(false);
        add(contentViewer);
    }

    private static StringBuilder calculateTooltip(JcrFsItem item) {
        ...
        if (item.isDirectory()) {
            ...
            if (!MavenUtils.isChecksum(name) && !MavenUtils.isMetadata(name)) {
                ttProps.put("downloads", Long.toString(file.getDownloadCount()));
            }
        }
        ttProps.put("deployed-by", item.getModifiedBy());
        //Format the tooltip
    ...
    private static String pathFromNodeId(String nodeId) {
        ...
        if (idx + 1 == nodeId.length()) {
            ...
            return Strings.fromEscapedUnicode(nodeId.substring(idx + 1));
        }
    }
