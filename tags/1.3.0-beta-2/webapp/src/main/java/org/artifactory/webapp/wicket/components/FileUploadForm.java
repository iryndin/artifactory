...
public class FileUploadForm extends Form {
    ...
    public FileUploadForm(String name, FileUploadParentPanel parent) {
        ...
        int uploadMaxSizeMb = CentralConfig.get().getFileUploadMaxSizeMb();
    ...
    protected void onSubmit() {
        ...
        if (upload != null) {
            ...
            try {
                //Save to a new file
                FileUtils.forceMkdir(ArtifactoryApp.UPLOAD_FOLDER);
                ...
                throw new IllegalStateException("Unable to write file to '"
                        + ArtifactoryApp.UPLOAD_FOLDER.getAbsolutePath() + "'.", e);
            }
        }
    }
    ...
    public boolean removeUploadedFile() {
        if (uploadedFile != null && uploadedFile.exists()) {
            ...
            if (!Files.remove(uploadedFile)) {
                LOGGER.warn("Unable to remove/overwrite " + uploadedFile.getAbsolutePath());
                return false;
            }
        }
        return true;
