...
public class CentralConfigServiceImpl implements CentralConfigService,
    ...
    public void importFrom(ImportSettings settings, StatusHolder status) {
        status.setStatus("Importing config...", LOGGER);
        File newConfigFile =
                new File(settings.getBaseDir(), ArtifactoryHome.ARTIFACTORY_CONFIG_FILE);
        if (newConfigFile.exists()) {
            status.setStatus("Reloading configuration from " + newConfigFile, LOGGER);
            reloadConfiguration(newConfigFile);
            init();
            status.setStatus("Configuration reloaded from " + newConfigFile, LOGGER);
        }
        status.setStatus("Importing repositories...", LOGGER);
        repositoryService.importFrom(settings, status);
        // TODO: Check for error before switching?
        status.setStatus("Switching config files...", LOGGER);
        if (newConfigFile.exists()) {
    ...
    public void exportTo(ExportSettings settings, StatusHolder status) {
        status.setStatus("Exporting config...", LOGGER);
