...
public class ActionsMenuPanel extends Panel {
    ...
    private final static Logger LOGGER = Logger.getLogger(ActionsMenuPanel.class);

    public ActionsMenuPanel(String id, final ActionableItemTreeNode node, ActionableItemsTree tree,
                            final Component menuTarget) {
        super(id, new Model(node));
        WebMarkupContainer menu = new WebMarkupContainer("menu");
        menu.add(new AttributeModifier("targetNodeIds", true,
                //Required since calling getMarkupId directly will result in a not yet attached to a
                //page exception
                new AbstractReadOnlyModel() {
                    @Override
                    public Object getObject() {
                        return menuTarget.getMarkupId();
                    }
                }));
        add(menu);
            ...
            protected void populateItem(ListItem item) {
                ...
                item.add(new AjaxEventBehavior("onClick") {
                    ...
                    protected void onEvent(AjaxRequestTarget target) {
                        Component targetComponent = descriptorTarget.getTarget();
                        ItemActionEvent event =
                                new ItemActionEvent(
                                        actionableItem, descriptor, targetComponent, target);
