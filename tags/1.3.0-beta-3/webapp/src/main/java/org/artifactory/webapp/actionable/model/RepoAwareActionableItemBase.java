...
import org.artifactory.webapp.actionable.view.GeneralTabPanel;
import org.artifactory.webapp.actionable.view.PermissionsTabPanel;
import org.artifactory.webapp.actionable.view.StatsTabPanel;
import org.artifactory.webapp.actionable.view.TabbedPanel;
...
public abstract class RepoAwareActionableItemBase extends ActionableItemBase
        implements RepoAwareActionableItem, TabbedViewActionableItem {
    ...
    public LocalRepoDescriptor getRepo() {
        ...
        LocalRepoDescriptor localRepo =
                getRepoService().localOrCachedRepoDescriptorByKey(repoKey);
        return localRepo;
    }
    ...
    public Panel createDispalyPanel(final String id) {
        Panel panel = new TabbedPanel(id) {
            @Override
    ...
    public void addTabs(List<ITab> tabs) {
        ...
        if (item.hasStatsInfo()) {
            tabs.add(new AbstractTab(new Model("Stats")) {
                @Override
                public Panel getPanel(String panelId) {
                    return new StatsTabPanel(panelId, item);
                }
            });
        }
