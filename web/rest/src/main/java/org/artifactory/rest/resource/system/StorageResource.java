...
public class StorageResource {
    ...
    public StorageResource(StorageService storageService, InternalBackupService backupService,
            InternalBinaryStore binaryStore, HttpServletResponse httpResponse,HttpServletRequest request) {
    ...
    public Response resetBinaryProviders() {
        Map<String, String[]> parameterMap = request.getParameterMap();
        String[] tasks = parameterMap.get("task");
        if(tasks==null|| tasks.length!=1){
            return Response.status(Response.Status.BAD_REQUEST).build();
        }
        switch (tasks[0]){
            case "reset":{
                Boolean result = binaryStore.getAndResetBinaryProvidersStatus(false);
                return Response.ok().entity(result).build();
            }
            case "balance":{
                binaryStore.balanceProviders();
