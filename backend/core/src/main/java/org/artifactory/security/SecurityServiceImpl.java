...
import org.artifactory.security.exceptions.*;
...
public class SecurityServiceImpl implements InternalSecurityService {
    ...
    private final Map<String, List<Long>> unknownUsersCache =
            Caches.newLRUCache(UserGroupServiceImpl.MAX_USERS_TO_TRACK);
    ...
    public void unlockUser(String userName) {
        if (!isAnonymousUser(userName)) {
            ...
            unknownUsersCache.remove(userName);
        }
    }
        ...
        if (!isAnonymousUser(userName)) {
            ...
            if (user == null) { // unknown user blocking
                ...
                if (!userGroupStoreService.isUserLocked(userName)) {
                    if(!unknownUsersCache.containsKey(userName)){
                        registerUnknownUser(userName);
                        // memorize incorrect login attempt
                        unknownUsersCache.get(userName).add(accessTime);
                    } else {
                        unknownUsersCache.get(userName).add(accessTime);
                        if (isUserLockPolicyEnabled() &&
                                unknownUsersCache.get(userName).size() >= getAllowedMaxLoginAttempts()) {
                            lockUser(userName);
                            unknownUsersCache.remove(userName); // no need to track this user as it got locked
                            return;
                        }
                    }
                }
    ...
    private int getPasswordExpirationDays() {
        PasswordExpirationPolicy passwordExpirationPolicy =
                configService.getMutableDescriptor()
                        .getSecurity().getPasswordSettings().getExpirationPolicy();
        return passwordExpirationPolicy.getExpiresIn();
    }
    ...
    public void ensureLoginShouldNotBeDelayed(String userName) {
        if(!isAnonymousUser(userName)) {
            ...
            if (nextLogin < 0 && unknownUsersCache.containsKey(userName)) {
                // check frontend cache for unknown users
                nextLogin = userGroupStoreService.getNextLogin(
                        unknownUsersCache.get(userName).size(),
                        unknownUsersCache.get(userName).get(unknownUsersCache.get(userName).size()-1)
                );
            }
    ...
    public void ensureSessionShouldNotBeDelayed(String sessionIdentifier) {
        if(!isAnonymousUser(sessionIdentifier)) {
            ...
            if (nextLogin < 0 && unknownUsersCache.containsKey(sessionIdentifier)) {
                // check frontend cache for unknown users
                nextLogin = userGroupStoreService.getNextLogin(
                        unknownUsersCache.get(sessionIdentifier).size(),
                        unknownUsersCache.get(sessionIdentifier).get(unknownUsersCache.get(sessionIdentifier).size()-1)
                );
            }
...
//    }
    ...
    public void expireUserCredentials(String userName) {
        try {
            ...
            UserInfo user = findUser(userName); // todo: [mp] use plain user fetch (rather than heavy groups join)
            if (Strings.isNullOrEmpty(user.getPassword())) {
                log.debug("User {} does not managed by system, ignoring expire request");
                throw new PasswordExpireException(
                        "It seems like '" + userName + "' is not managed by the artifactory, expiring credentials is not possible"
                );
            }
    ...
    public void unExpireUserCredentials(String userName) {
        try {
            ...
            userGroupStoreService.unExpireUserPassword(userName);
        } catch (StorageException e) {
            throw new PasswordExpireException("Expiring password for \"" + userName + "\" has failed, " + e.getMessage(), e);
        } catch (UsernameNotFoundException e) {
            log.error(e.getMessage());
    ...
    public void unExpireCredentialsForAllUsers() {
        try {
            userGroupStoreService.unExpirePasswordsForAllUsers();
        } catch(StorageException e) {
    ...
    public Map<UserInfo, Long> getUsersWitchPasswordIsAboutToExpire() {
        return userGroupStoreService.getUsersWitchPasswordIsAboutToExpire(
                ConstantValues.passwordBeforeExpireNotificationStartInDays.getInt(),
