...
public class TrafficServiceImpl implements InternalTrafficService {
    ...
    public void init() {
        ...
        vfsService.createIfNeeded(getLogFolderPath());
    ...
    private void handleTrafficEntryInternal(TrafficEntry entry, ArtifactorySession session) {
        //Store a log entry and notify the queue
        String entryPath = generateEntryPath();
        try {
            MutableVfsNode rootNode = (MutableVfsNode) session.getVfsRootNode();
            while (rootNode.hasChild(entryPath)) {
                //We are extremely(!) lucky
                entryPath = generateEntryPath();
            }
            final MutableVfsNode entryNode = rootNode.getOrCreateSubNode(entryPath, VfsNodeType.LOG);
            final Calendar calendar = Calendar.getInstance();
            calendar.setTime(entry.getDate());
            entryNode.setProperty(StorageConstants.PROP_ARTIFACTORY_TIMESTAMP, calendar);
            //Store the serialized entry
            entryNode.setProperty(StorageConstants.PROP_ARTIFACTORY_TRAFFIC_ENTRY, entry.toString());
    ...
    public void collect() {
        ...
        ArtifactorySession session = null;
        try {
            session = vfsService.getUnmanagedSession();
            if (readyCount > 0) {
            ...
            log.error("Error collecting traffic entries " + e.getMessage(), e);
        } finally {
            if (session != null) {
                session.logout();
            }
     ...
     */
    public TrafficEntriesIterator getDatabaseEntries(
            @Nullable Calendar from, @Nullable Calendar to, ArtifactorySession session) {
        VfsQuery vfsQuery = vfsQueryService.createQuery();
        vfsQuery.addRelativePathFilter(getLogFolderPath());
        vfsQuery.setNodeTypeFilter(VfsNodeType.LOG);
        vfsQuery.addCriterion(StorageConstants.PROP_ARTIFACTORY_TIMESTAMP, VfsComparatorType.GREATER_THAN_EQUAL, from);
        vfsQuery.addCriterion(StorageConstants.PROP_ARTIFACTORY_TIMESTAMP, VfsComparatorType.LOWER_THAN, to);
        vfsQuery.orderByAscending(StorageConstants.PROP_ARTIFACTORY_TIMESTAMP);
        VfsQueryResult vfsQueryResult = vfsQuery.execute(false);
        Iterable<VfsNode> nodes = vfsQueryResult.getNodes();
        final TrafficEntriesIterator trafficEntriesIterator =
                new TrafficEntriesIterator(nodes, vfsQueryResult.getCount());
     ...
     */
    public Calendar getLastCollected(@Nullable TrafficCollectorResolution resolution, ArtifactorySession session) {
        final VfsNode logFolder = getLogFolderNode(session);
        Calendar lastCollected = null;
        String propName = StorageConstants.PROP_ARTIFACTORY_LAST_COLLECTED;
        if (resolution != null) {
            propName = StorageConstants.PROP_ARTIFACTORY_LAST_COLLECTED + resolution.getName();
        }
    ...
    public void updateLastCollected(@Nullable TrafficCollectorResolution resolution,
            Calendar lastCollected, ArtifactorySession session) {
        final MutableVfsNode logFolder = getLogFolderNode(session);
        String propName = StorageConstants.PROP_ARTIFACTORY_LAST_COLLECTED;
        if (resolution != null) {
            propName = StorageConstants.PROP_ARTIFACTORY_LAST_COLLECTED + resolution.getName();
        }
        logFolder.setProperty(propName, lastCollected);
    }
    ...
    private MutableVfsNode getLogFolderNode(ArtifactorySession session) {
        MutableVfsNode rootNode = (MutableVfsNode) session.getVfsRootNode();
        return rootNode.getOrCreateSubNode(getLogFolderPath(), VfsNodeType.LOG);
    }

    private String getLogFolderPath() {
        return PathFactoryHolder.get().getLogPath(LOG_FOLDER);
    }
