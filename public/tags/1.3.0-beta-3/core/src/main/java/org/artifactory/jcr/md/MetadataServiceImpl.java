...
public class MetadataServiceImpl implements MetadataService {
    ...
    public <MD> MD getLockedXmlMetadataObject(MetadataAware obj, Class<MD> clazz) {
        ...
        value.lock();
        return (MD) value.getMetadataValue();
    }
            ...
            if (value != null) {
                ...
                if (value.isTransient()) {
                    ...
                    if (!metadataContainer.hasNode(metadataName)) {
                        ...
                        if (value.getMetadataValue() != null) {
    ...
    private void setXmlMetadata(MetadataAware metadataAware, String metadataName, InputStream is,
        ...
        try {
            ...
            MetadataDefinition definition =
                    definitionService.getMetadataDefinition(metadataName);
            synchronized (cache) {
                ...
                //TODO: [by yl] To avoid JCR-1554 we should create the node, unless it already exists
                boolean exists = metadataContainer.hasNode(metadataName);
                if (exists) {
                    //Remove any old metadata node first
                    Node metadataNode;
                    try {
                        metadataNode = metadataContainer.getNode(metadataName);
                        metadataNode.remove();
                    } catch (RepositoryException e) {
                        //In case another thread just removed the node under us
                        LOGGER.warn("Failed to set xml data: " + e.getMessage());
                        return;
                    }
                }
                if (is == null) {
            ...
            Node metadataNode = metadataContainer.addNode(metadataName);
            ...
            //Add the xml
            Node xmlNode = metadataNode.addNode(MetadataAware.NODE_ARTIFACTORY_XML);
            importXml(xmlNode, is);
            is.reset();
            //Add the original raw xml
            Node resourceNode = metadataNode.addNode(JCR_CONTENT, NT_RESOURCE);
    ...
    private static MetadataValue createEmptyValue(
            MetadataKey key) {
        MetadataValue value;
        try {
            ...
            throw new RepositoryRuntimeException(
                    "Request to create default persistent metadata for " +
                            key + " failed.", e);
        }
        return value;
    }
