...
public class ArtifactGraphVisitor implements ArtifactVisitor {
    ...
    private static Logger logger = Logger.getLogger(ArtifactGraphVisitor.class);
    ...
    public void visitDependencies(List<ArtifactDependency> dependencies, Artifact dependent) {
        increaseDepth();
        if (!isDepthAllowed()) {
            decreaseDepth();
            logger.debug(
