...
public class LocalCacheRepo extends JcrRepo {
    ...
    public void undeploy(String path) {
        ...
        int itemsEffected = processSubNodesCleanup(path, true);
    ...
    public void expire(String path) {
        ...
        int itemsEffected = processSubNodesCleanup(path, false);
        if (LOGGER.isInfoEnabled()) {
    ...
    private int processSubNodesCleanup(final String path, final boolean remove) {
        JcrHelper jcr = getJcr();
        Integer itemsEffected = jcr.doInSession(new JcrCallback<Integer>() {
            ...
            public Integer doInJcr(JcrSessionWrapper session) throws RepositoryException {
                ...
                Node folderNode;
                if (path.length() > 0) {
                    folderNode = repoNode.getNode(path);
                } else {
                    //We are processing the root repo node
                    folderNode = repoNode;
                }
                JcrFolder folder = new JcrFolder(folderNode);
                int itemsCount = expireSubNodes(folder, 0, remove);
    ...
    private int expireSubNodes(JcrFolder folder, int itemsCount, boolean partOfRemove) {
        List<JcrFsItem> list = folder.getItems();
        for (JcrFsItem item : list) {
            if (item.isDirectory()) {
                itemsCount = expireSubNodes((JcrFolder) item, itemsCount, partOfRemove);
            } else {
