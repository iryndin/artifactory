...
public class UserDatumVertexPaintFunction implements VertexPaintFunction {
    ...
    public Paint getFillPaint(Vertex vertex) {
        Integer depth = (Integer) vertex.getUserDatum(JungGraph.TRANSITIVE_DEPTH);
        if (depth == 0) {
            return Color.YELLOW;
        } else if (vertex.getUserDatum(JungGraph.DUMMY_VERTEX) != null) {
            return Color.GRAY;
        } else {
            int g = ((depth - 1) * 25);
            if (g > 255) g = 255;
    ...
    public Paint getDrawPaint(Vertex vertex) {
        if ((Integer) vertex.getUserDatum(JungGraph.TRANSITIVE_DEPTH) == 0) {
            return Color.ORANGE;
        } else if (vertex.getUserDatum(JungGraph.DUMMY_VERTEX) != null) {
