...
public class LocalCacheRepo extends JcrRepoBase<LocalCacheRepoDescriptor> {
    ...
    public StatusHolder assertValidPath(RepoPath repoPath) {
        //If the remote key is being used - check against the full cache key
        String remoteRepoKey = getRemoteRepo().getKey();
        if (remoteRepoKey.equals(repoPath.getRepoKey())) {
            repoPath = new RepoPath(getKey(), repoPath.getPath());
        }
        return super.assertValidPath(repoPath);
    }
