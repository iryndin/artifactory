...
public class IndexerManagerImpl implements IndexerManager {
    private static final Logger log = LoggerFactory.getLogger(IndexerManagerImpl.class);
    ...
    public void register() {
        InternalContextHelper.get().addReloadableBean(IndexerManager.class);
    }
    ...
    public static class FindOrCreateIndexJob extends QuartzCommand {
        ...
        protected void onExecute(JobExecutionContext callbackContext) {
            try {
                ...
                IndexerManager manager = InternalContextHelper.get().beanForType(IndexerManager.class);
                manager.findOrCreateIndex(repoIndexerData, fireTime);
            } catch (Exception e) {
        ...
        protected void onExecute(JobExecutionContext callbackContext) {
            try {
                RepoIndexerData repoIndexerData =
                        (RepoIndexerData) callbackContext.getMergedJobDataMap().get(RepoIndexerData.class.getName());
                IndexerManager manager = InternalContextHelper.get().beanForType(IndexerManager.class);
                manager.saveIndexFiles(repoIndexerData);
            } catch (Exception e) {
