...
public abstract class ArtifactoryHome {
    ...
    public static File getTmpDataDir() {
        return tmpDataDir;
    }

    public static void setJcrRootDir(File jcrRootDir) {
        ArtifactoryHome.jcrRootDir = jcrRootDir;
    }
    ...
    public static void create() {
        try {
            ...
            jcrRootDir = dataDir;
            tmpDataDir = getOrCreateSubDir(dataDir, "tmp");
        } catch (IOException e) {
            throw new IllegalArgumentException(
                    "Could not create or access artifactory main directory " + e.getMessage(), e);
        }
        String configFilePath =
                System.getProperty(SYS_PROP_ARTIFACTORY_CONFIG);
        if (configFilePath == null) {
            configFile = new File(etcDir, ARTIFACTORY_CONFIG_FILE);
        ...
        File artifactoryPropertiesFile = new File(etcDir, ARTIFACTORY_PROPERTIES_FILE);
        if (!artifactoryPropertiesFile.exists()) {
            try {
                //Copy from default
                URL url = ArtifactoryHome.class
                        .getResource("/META-INF/default/" + ARTIFACTORY_PROPERTIES_FILE);
                FileUtils.copyURLToFile(url, artifactoryPropertiesFile);
            } catch (IOException e) {
                throw new RuntimeException("Could not create a default " +
                        ARTIFACTORY_PROPERTIES_FILE + " at " +
