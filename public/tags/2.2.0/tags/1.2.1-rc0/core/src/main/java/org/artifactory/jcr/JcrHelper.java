...
public class JcrHelper implements DisposableBean {
    ...
    public static void importStream(
            ...
            throws RepositoryException, IOException {
        //Lock both the parent and the grandparent nodes in order to avoid a situation of pending
        //changes on the parent node (we locked a child folder first in the same transaction).
        //For example, the group metadata.xml is deployed (by the deployer) after the version
        //metadata and the after the artifacts. We cannot unlock the children without saving them
        //first (pending changes) which will compromise the atomicy of the transaction.
        //Do not go up with locking more than 2 levels in order no to fail on pending changes
    ...
    public static Node createFolder(Node parentNode, String folderName, String repoKey)
        ...
        Node folderNode;
        //Do not lock the roor node - it is not lockable
        if (!rootNode) {
            NodeLock.lock(parentNode);
        }
