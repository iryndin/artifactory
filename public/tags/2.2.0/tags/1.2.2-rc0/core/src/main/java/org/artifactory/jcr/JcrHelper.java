...
public class JcrHelper implements DisposableBean {
    ...
    public static void importStream(
        ...
        CentralConfig cc = ContextUtils.getContext().getCentralConfig();
        ...
        if (!repo.handles(relPath)) {
            throw new RepositoryException(
                    "The repository '" + repoKey + "' does not handle the artifact at '" + relPath +
                            "'. Some files might have been incorrectly imported - Please remove " +
                            "them manually.");
        }
        if (!repo.accepts(relPath)) {
            throw new RepositoryException(
                    "The repository '" + repoKey + "' does not accept the path '" + relPath +
                            "'. Some files might have been incorrectly imported - Please remove " +
                            "them manually.");
        }
