...
public class JcrRepo extends RepoBase implements LocalRepo {
    ...
    public static String SYS_PROP_JCR_FORCE_ATOMIC_TX = "artifactory.jcr.forceAtomicTx";
    ...
    public ResourceStreamHandle getResourceStreamHandle(final RepoResource res) throws IOException {
        final String absPath = "/" + repoPath + "/" + res.getPath();
    ...
    public void importFromDir(final File sourceDir, boolean singleTransaction) {
        if (sourceDir == null || !sourceDir.exists() || !sourceDir.isDirectory()) {
            throw new RuntimeException(
                    "Cannot import null, non existent folder or non directory file '" + sourceDir +
                            "'.");
        }
        final String basePath = sourceDir.getPath();
        if (singleTransaction && !forceAtomicTransacions) {
            jcr.doInSession(new JcrCallback<Node>() {
                public Node doInJcr(JcrSessionWrapper session) throws RepositoryException {
                    Node repoNode = getRepoJcrNode(session);
                    try {
        ...
        try {
            Node resNode = jcr.doInSession(new JcrCallback<Node>() {
                ...
                public Node doInJcr(JcrSessionWrapper session) throws RepositoryException {
                    ...
                    String resDirAbsPath = "/" + repoPath + "/" + resPath.substring(0, idx);
            ...
            localRepoInterceptor.afterSaveResource(res, jcrFile);
        } catch (Exception e) {
    ...
    public boolean fileNodeExists(String path) {
        return jcr.fileNodeExists("/" + repoPath + "/" + path);
    }
    ...
    private void importFile(Node parentNode, File dirEntry, String basePath)
        ...
        Node childNode = JcrHelper.importStream(parentNode, dirEntry.getName(), key,
                dirEntry.lastModified(), bis, false);
        //Update the metadata
