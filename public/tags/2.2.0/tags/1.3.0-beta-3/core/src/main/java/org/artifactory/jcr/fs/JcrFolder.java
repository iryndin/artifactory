...
public class JcrFolder extends JcrFsItem<FolderInfo> {
    ...
    public void exportTo(ExportSettings settings, StatusHolder status) {
        try {
            ...
            status.setStatus("Exporting directory '" + relPath + "'...");
            ...
            String msg;
            if (exportDir != null) {
                msg = "Failed to export to dir '" + exportDir.getPath() + "'. " + e.getMessage();
            } else {
                msg = "Failed to export to a null dir: " + e.getMessage();
            }
    ...
    public void importFrom(ImportSettings settings, StatusHolder status) {
        ...
        try {
            ...
            String msg = "Failed to import folder into '" + getRelativePath() + "'.";
            status.setError(msg, e);
        }
    }
            ...
            for (File dirEntry : dirEntries) {
                ...
                if (dirEntry.isDirectory() && isStorable(fileName)) {
                    ...
                    try {
                        ...
                        //Just log an error and continue
                        status.setError("Could not import file " + dirEntry, e, LOGGER);
                    }
                }
            }
        } catch (Exception e) {
