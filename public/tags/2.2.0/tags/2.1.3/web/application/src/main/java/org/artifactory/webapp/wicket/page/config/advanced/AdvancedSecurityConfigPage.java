            ...
            protected void onSubmit(AjaxRequestTarget target, Form form) {
                ...
                if (StringUtils.isEmpty(editorValue)) {
                    ...
                    try {
                        ...
                        SecurityInfo descriptorToSave = ((SecurityInfo) xstream.fromXML(editorValue));
                        securityService.importSecurityData(descriptorToSave);
                        info("Security configuration successfully saved.");
                    } catch (Exception e) {
                        log.debug("Error while manually saving the security configuration.", e);
                        error("An error has occured while saving the security configuration (" + e.getMessage() +
