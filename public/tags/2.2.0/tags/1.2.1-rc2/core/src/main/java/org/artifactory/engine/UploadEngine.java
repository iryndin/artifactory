...
public class UploadEngine {
    ...
    public void process(ArtifactoryRequest request, ArtifactoryResponse response)
        ...
        if (snapshot && !repo.isUseSnapshotUniqueVersions() && realArtifact) {
            ...
            path = path.substring(0, lastSepIdx + 1) + artifactId + "-" + version + "." + type;
        }
        InputStream is = null;
        if (snapshot && !repo.isUseSnapshotUniqueVersions()) {
            if (path.endsWith("-SNAPSHOT/maven-metadata.xml")) {
                ...
                //artifact (otherwise they will try to locate a file name that has the timestamp in it).
            ...
            } else if (path.endsWith("-SNAPSHOT/maven-metadata.xml.md5")) {
                //We end up with broken signatures here since we crafted the metadtata:
                //For version metadata checksums ignore what the client is sending and calc the
                //metadata on what is stored in jcr
                String metadataPath = path.substring(0, path.length() - 4);
                String checksum = getChecksum(repo, metadataPath, "MD5");
                is = new ByteArrayInputStream(checksum.getBytes("ISO-8859-1"));
            } else if (path.endsWith("-SNAPSHOT/maven-metadata.xml.sha1")) {
                String metadataPath = path.substring(0, path.length() - 5);
                String checksum = getChecksum(repo, metadataPath, "SHA-1");
                is = new ByteArrayInputStream(checksum.getBytes("ISO-8859-1"));
            }
        }
        if (is == null) {
    ...
    private String getChecksum(LocalRepo repo, String metadataPath, String algorithm)
            throws IOException, NoSuchAlgorithmException {
        SimpleRepoResource metadataRes = new SimpleRepoResource(repo, metadataPath);
        ResourceStreamHandle handle = repo.getResourceStreamHandle(metadataRes);
        ChecksumObserver checksumObserver = new ChecksumObserver(algorithm);
        String cheksum;
        try {
            InputStream metadataStream = handle.getInputStream();
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            IOUtils.copy(metadataStream, baos);
            checksumObserver.transferStarted(null);
            checksumObserver.transferProgress(null, baos.toByteArray(), baos.size());
            checksumObserver.transferCompleted(null);
            cheksum = checksumObserver.getActualChecksum();
        } finally {
            handle.close();
        }
        return cheksum;
