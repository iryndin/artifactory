if (typeof dojo == "undefined") {
    ...
    */
    ...
    dojo.hostenv.localesGenerated /***BUILD:localesGenerated***/;
...
if (typeof window != 'undefined') {
    ...
    (function() {
        ...
        if (djConfig.allowQueryConfig) {
            ...
            // FIXME: use location.query instead?
...
// we're omitting a progress() event for now
...
dojo.io.Error = function(/*String*/ msg, /*String*/ type, /*Number*/num) {
    ...
    // must be one of "io", "parse", "unknown"
    this.number = num || 0;
    // per-substrate error number, not normalized
}
...
dojo.dom.destroyNode = function(/*Node*/node) {
    ...
    if (node.nodeType != 3) { // ingore TEXT_NODE
        ...
        if (dojo.render.html.ie) {
            node.outerHTML = '';
            //prevent ugly IE mem leak associated with Node.removeChild (ticket #1727)
        }
    }
}
...
if (!dj_undef("window")) {
    ...
    dojo.io.XMLHTTPTransport = new function() {
        ...
        var _cache = {};
        // FIXME: make this public? do we even need to?
        this.useCache = false;
        // if this is true, we'll cache unless kwArgs.useCache = false
        this.preventCache = false;
        // if this is true, we'll always force GET requests to cache
...
dojo.event.topic = new function() {
    ...
    this.getTopic = function(/*String*/topic) {
        ...
        // a dojo.event.topic.TopicImpl object
    }
...
// cache container needed because IE reformats cssText when added to DOM
...
dojo.html.show = function(/* HTMLElement */node) {
    ...
    if (dojo.html.getStyleProperty(node, 'display') == 'none') {
        ...
        // cannot use delete on a node in IE6
    }
}
    ...
    if (h.ie) {
        try {
            if (node.filters && node.filters.alpha) {
                ns.filter = "";
                // FIXME: may get rid of other filter effects
            }
        } catch(e) {
            /*
...
dojo.html.getAbsolutePosition = dojo.html.abs = function(/* HTMLElement */node, /* boolean? */includeScroll, /* string? */boxType) {
    ...
    if (h.ie) {
        ...
        // mozilla
        nativeBoxType = 1;
        //getBoxObjectFor return padding box coordinate
        try {
...
dojo.lfx.html.wipeIn = function(/*DOMNode[]*/ nodes, /*int?*/ duration, /*Function?*/ easing, /*Function?*/ callback) {
    ...
    dojo.lang.forEach(nodes, function(node) {
        var oprop = {  };
        // old properties of node (before we mucked w/them)
