...
public abstract class NodeLock {
    ...
    public static void lock(Node node) {
        lock(node, ArtifactoryConstants.maxNodeLockRetries, 1000);
    }
        ...
        while (true) {
            try {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Locked node '" + path + "' with session " + session);
                }
                return;
                ...
                if (retries > 0) {
                    ...
                    throw new RuntimeException(
                            "Failed to lock '" + path + "' after " + maxRetries + " retries.");
                }
            } catch (RepositoryException e) {
                throw new RuntimeException("Failed to lock '" + path + "'.", e);
            }
        }
    }
    ...
    public static void unlock(Node node) {
        ...
        try {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug("Failed to unlock '" + path + "'.", e);
            } else {
                LOGGER.warn("Failed to unlock '" + path + "'.");
            }
        }
    }

    public static boolean isLockedByCurrentSession(Node node) throws RepositoryException {
        if (!node.isLocked()) {
            return false;
        }
