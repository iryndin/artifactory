...
public class GavcSearchPanel extends BaseSearchPanel<GavcSearchResult> {
    ...
    private void addCheckboxLabel(StyledCheckbox checkbox) {
        checkbox.setLabel(new Model("Exact Match"));
    }
    ...
    protected void onNoResults() {
        Session.get().warn(String.format("No artifacts found for '%s'.",
                StringEscapeUtils.escapeHtml(searchControls.getSearchExpression())));
    }
