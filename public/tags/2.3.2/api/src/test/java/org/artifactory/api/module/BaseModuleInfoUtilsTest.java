...
public abstract class BaseModuleInfoUtilsTest {
    ...
    protected void testArtifactModuleToModule(ModuleInfo moduleInfo) {
        String artifactPath = ModuleInfoUtils.constructArtifactPath(moduleInfo, repoLayout);
        ModuleInfo extractedModuleInfo = ModuleInfoUtils.moduleInfoFromArtifactPath(artifactPath, repoLayout);
    ...
    protected void testArtifactPathToModule(String artifactPath, ModuleInfo expectedModule, RepoLayout repoLayout) {
        ModuleInfo extractedModuleInfo = ModuleInfoUtils.moduleInfoFromArtifactPath(artifactPath, repoLayout);
    ...
    protected void testDescriptorPathToModule(String descriptorPath, ModuleInfo expectedModule, RepoLayout repoLayout) {
        ModuleInfo extractedModuleInfo = ModuleInfoUtils.moduleInfoFromDescriptorPath(descriptorPath, repoLayout);
        assertEquals(extractedModuleInfo, expectedModule, "Converted module info does not match the expected one.");
    }
}
