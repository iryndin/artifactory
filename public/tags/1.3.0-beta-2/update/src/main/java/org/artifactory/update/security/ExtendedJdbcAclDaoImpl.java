...
public class ExtendedJdbcAclDaoImpl extends JdbcExtendedDaoImpl implements ExtendedJdbcAclDao {
    ...
    public static final String ORG_ACEGISECURITY = "org.acegisecurity";
    public static final String ORG_SPRINGFRAMEWORK_SECURITY = "org.springframework.security";
    ...
    public void deleteAcls(String recipient) {
        //TODO: [by yl] Cache cleanup too coarse
        aclProvider.clearAclEntryCache();
        deleteAclsByRecipient.update(recipient);
    }

    public void deleteAcls(RepoPath identity) {
        aclProvider.getBasicAclEntryCache().removeEntriesFromCache(identity);
    ...
    public void update(SimpleAclEntry aclEntry) {
        ...
        create(aclEntry);
        aclProvider.getBasicAclEntryCache().putEntriesInCache(new SimpleAclEntry[]{aclEntry});
    }
    ...
    protected class GetAllIdentitiesMapping extends MappingSqlQuery {
        ...
        protected Object mapRow(ResultSet rs, int rownum)
            ...
            try {
                // Replace the package names to new security
                if (aclClass.startsWith(ORG_ACEGISECURITY)) {
                    aclClass = ORG_SPRINGFRAMEWORK_SECURITY + aclClass.substring(ORG_ACEGISECURITY.length());
                }
