...
public abstract class HttpUtils {
    ...
    /**
     * Extracts session access time
     *
     * @param servletRequest
     * @return session access time
     */
    public static long getSessionAccessTime(HttpServletRequest servletRequest) {
        return servletRequest.getSession().getLastAccessedTime() == 0 ?
                servletRequest.getSession().getCreationTime()
                :
                servletRequest.getSession().getLastAccessedTime();
    }
}
