...
public class ArtifactResource extends SimpleRepoResource {
    ...
    public ArtifactResource(
        ...
        super(getPath(groupId, artifactId, version, classifier, packagingType), repo);
    ...
    public ArtifactResource(String path, Repo repo) {
        super(path, repo);
        init();
    }
    ...
    private void init() {
        ...
        if (!metaData && !hash) {
            ...
            if (!snapshot) {
                ...
                //RTFACT-96: When having a <uniqueVersion> flag in the users POM -
                //If set to "false" the name will end with "-SNAPSHOT", else will end with
                //"-<numerical-version-timestamp>"
                if (!Character.isDigit(name.charAt(idx + 1))
                        && !MavenUtils.isVersionSnapshot(name)) {
                    classifier = name.substring(
                            idx + 1, //After the '-'
                            name.length() - 1 - type.length()//Remove the .[type]
                    );
