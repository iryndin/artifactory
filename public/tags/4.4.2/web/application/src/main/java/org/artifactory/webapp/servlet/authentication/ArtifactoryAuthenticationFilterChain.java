    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, final FilterChain servletChain)
        ...
        for (ArtifactoryAuthenticationFilter filter : this.authenticationFilters) {
            if (filter.acceptFilter(servletRequest)) {
                ...
                try {
                    loginIdentifier = getLoginIdentifier(servletRequest, filter);
                    if (loginIdentifier == null) {
                        ...
                        if(Strings.isNullOrEmpty(loginIdentifier)) {
                            ...
                            // makes sure that user credentials have not expired
                            // (we do not lock user on exp. credentials as per product req.)
                            if(!HttpUtils.isChangePasswordRequest(servletRequest))
                                getSecurityService().ensurePasswordIsNotExpired(loginIdentifier);
                ...
                } catch (UserCredentialsExpiredException e) {
                    log.debug("User credentials have expired, {}", e);
