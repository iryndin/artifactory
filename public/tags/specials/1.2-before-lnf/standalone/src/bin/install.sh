...
echo
echo -n "Setting file permissions to etc, logs, repos and backup..."
...
mkdir -p ${ARTIFACTORY_HOME}/backups/ && \
chown ${JETTY_USER} ${ARTIFACTORY_HOME}/backups/ && \
chmod u+w ${ARTIFACTORY_HOME}/backups/ && \
mkdir -p ${ARTIFACTORY_HOME}/repos/ && \
chown ${JETTY_USER} -R ${ARTIFACTORY_HOME}/repos/ && \
chmod u+w -R ${ARTIFACTORY_HOME}/repos/ && \
permChangeOK=true
