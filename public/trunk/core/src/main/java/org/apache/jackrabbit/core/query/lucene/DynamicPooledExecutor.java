...
/**
 * <code>DynamicPooledExecutor</code> implements an executor, which dynamically adjusts its maximum number of threads
 * according to the number of available processors returned by {@link Runtime#availableProcessors()}.
 */
public class DynamicPooledExecutor {
    ...
    public DynamicPooledExecutor() {
        ...
        executor.setKeepAliveTime(120000);
        adjustPoolSize();
    }

    /**
     * Executes the given command. This method will block if all threads in the pool are busy and return only when the
     * command has been accepted. Care must be taken, that no deadlock occurs when multiple commands are scheduled for
     * execution. In general commands should not depend on the execution of other commands!
     *
    ...
    /**
     * Executes a set of commands and waits until all commands have been executed. The results of the commands are
     * returned in the same order as the commands.
     *
