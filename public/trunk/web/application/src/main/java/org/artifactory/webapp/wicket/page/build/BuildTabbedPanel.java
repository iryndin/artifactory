...
import org.artifactory.webapp.wicket.page.build.tabs.BuildInfoXmlTabPanel;
...
public class BuildTabbedPanel extends TitledPanel {
    ...
    private void addTabbedPanel() {
        ...
        tabList.add(getXmlTab("Build Info XML", hasDeployOnLocal));
    ...
    /**
     * Returns the info XML tab
     *
     * @param title   Title to give to the tab
     * @param enabled Indicates the state of the tab
     * @return Info XML tab
     */
    private ITab getXmlTab(String title, boolean enabled) {
        if (enabled) {
            return new BaseTab(title) {
                @Override
                public Panel getPanel(String panelId) {
                    return new BuildInfoXmlTabPanel(panelId, build);
                }
            };
        } else {
            return getDisabledTab(title);
