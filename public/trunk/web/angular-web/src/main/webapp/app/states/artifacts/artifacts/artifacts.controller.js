        ...
        this.setMeUpScope.snippets = {
            ...
            pypi: {
                read: [{
                    ...
                    after: "If credentials are required they should be embedded in the URL. To resolve packages using pip, run: "
                }, {
                    before: "To resolve packages using pip",
            ...
            bower: {
                read: [{
                    before: "In order to use Bower with Artifactory you need 2 components (npm packages):<br />" + "  1. <a href=\"https://www.google.com/url?q=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fbower-art-resolver&sa=D&sntz=1&usg=AFQjCNH5pnW2E2ETaXtmJL33xBhGkxKPag\">bower-art-resolver</a> - A custom Bower resolver dedicated to integrate with Artifactory.<br />" + "  2. <a href=\"https://www.google.com/url?q=https%3A%2F%2Fwww.npmjs.com%2Fpackage%2Fbower-art&sa=D&sntz=1&usg=AFQjCNGkNCqpo9TChhY7W-ZJb69Rk2sczg\">bower-art</a> - A temporary custom Bower CLI with the pluggable resolvers mechanism.\n" + "The bower-art package is a peer dependency of bower-art-resolver. Therefore, both can be easily installed with:",
                    snippet: "npm install -g bower-art-resolver"
                }, {
                    before: "To replace the default Bower registry add the following to your <i>.bowerrc</i> file",
                    snippet: "{\n" + " \"registry\" : \"$2/api/bower/$1\"\n" + "}"
                }, {
                    before: "If authentication is required use:",
                    snippet: "{\n" + " \"registry\" : \"http://<USERNAME>:<API_KEY>@$4/artifactory/api/bower/$1\"\n" + "}"
                }, {
                    before: "Execute the following command",
                    snippet: "bower-art install <PACKAGE>"
                }],
                deploy: {
                    before: "To deploy a Bower package into an Artifactory repository you need to use Artifactory's REST API or the Web UI.  For example, to deploy a Bower package into this repository using the REST API you would use:",
            ...
            nuget: {
                read: [{
                    before: "NuGet repositories must be prefixed with api/nuget in the path. For example, when configuring Visual Studio to access a NuGet repository in Artifactory, the repository URL must be prefixed with <b>api/nuget</b> in the path like this:",
                ...
                deploy: [{
                    ...
                    before: "You can also add Artifactory repository as a gallery by running the following command",
                    snippet: "nuget sources Add -Name Artifactory -Source $2/api/nuget/$1",
                    after: "This will enable you to use NuGet from command line to both push to and pull from Artifactory."
                }]
            },
            ivy: {
                general: {
...
//            var url = new URL(data.baseUrl) //CAUSES PROBLEM ON IE, NOT REALY NEEDED...
    ...
    selectNode(node) {
        ...
        if (node.data) {        
            ...
            let location = true;
            if (this.$state.current.name === 'artifacts.browsers.path' && !previousNode) {
