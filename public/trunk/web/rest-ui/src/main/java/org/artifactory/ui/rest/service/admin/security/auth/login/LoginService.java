...
public class LoginService extends AbstractLoginService {
    ...
    public void doExecute(ArtifactoryRestRequest request, RestResponse response) {
        ...
        // check if credentials valid (from expiration PoV)
        if(!HttpUtils.isChangePasswordRequest(request.getServletRequest())) {
            try {
                securityService.ensurePasswordIsNotExpired(userName);
            } catch (UserCredentialsExpiredException uce) {
                response.error(uce.getMessage()).responseCode(HttpStatus.SC_FORBIDDEN);
                response.iModel(new CredentialsExpiredFailedLoginResponse(userGroupService.findUser(userName).isUpdatableProfile()));
                return;
            }
        }
