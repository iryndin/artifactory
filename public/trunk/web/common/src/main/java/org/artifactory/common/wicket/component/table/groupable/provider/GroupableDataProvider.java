...
public class GroupableDataProvider<T extends Serializable> extends SortableDataProvider implements IGroupStateLocator {
    ...
    public Iterator iterator(int first, int count) {
        ...
        SortParam sortBy = getSort();

        //Null sort param and valid group param means that the group column is also the sorting column
        if ((sortBy == null) && (groupParam != null)) {
            sortBy = groupParam;
        }
        ListPropertySorter.sort(data, sortBy);
