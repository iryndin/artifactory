...
public class UserGroupServiceImpl implements UserGroupStoreService {
    ...
    public void updateUser(MutableUserInfo userInfo) {
        try {
            ...
            if (originalUser.isCredentialsExpired() == updatedUser.isCredentialsExpired() && // admin not changing password expired state
                ...
                userGroupsDao.unExpireUserPassword(originalUser.getUsername());
            }
        } catch (SQLException e) {
    ...
    public void unExpireUserPassword(String userName) {
        try {
            if (getUser(userName) != null) {
                userGroupsDao.unExpireUserPassword(userName);
                return;
            }
            throw new UsernameNotFoundException("User " + userName + " is not exist");
        } catch (SQLException e) {
     ...
     */
    @Override
    public void unExpirePasswordsForAllUsers() {
        try {
     ...
     *
     * @param daysToStartNotificationsFrom amount of days before password expiry to start notifying users
     * @param daysToKeepPassword           after what period password should be changed
     * @return list of users
     */
    @Override
    public Map<UserInfo, Long> getUsersWitchPasswordIsAboutToExpire(int daysToStartNotificationsFrom,
            int daysToKeepPassword) {
        final int secondsInDay = 86400;
        final int millisInDay = secondsInDay * 1000;

        long millisDaysToStartNotificationsFrom = daysToStartNotificationsFrom * millisInDay;
        long millisDaysToKeepPassword = daysToKeepPassword * millisInDay;
        long millisNow = DateTime.now().getMillis();

        try {
            return userGroupsDao.getUsersWitchPasswordIsAboutToExpire(millisNow, millisDaysToStartNotificationsFrom,
                    millisDaysToKeepPassword);
        } catch (SQLException e) {
    ...
    public void markUsersCredentialsExpired(int daysToKeepPassword) {
        ...
        try {
            userGroupsDao.markCredentialsExpired(millisDaysToKeepPassword);
        } catch (SQLException e) {
