 ...
 * @author freds
 */
public class ArtifactoryDbDataStoreImpl extends ArtifactoryDataStore implements DatabaseAware {
    ...
    public class FilesLruCache {
        ...
        /**
         * Cache semaphore for protecting cleanup and initialization work. Allows only one thread to do a cleanup or
         * initialization.
         */
        ...
        private FilesLruCache(String homeDir) throws DataStoreException {
            ...
            this.executor = StorageContextHelper.get().beanForType(CachedThreadPoolTaskExecutor.class);
        }
        ...
        private void clean() {
            ...
            // The following try catch block is to ensure that if we fail to start the cleanup task then
            // we have to release the cacheInitAndCleanLock semaphore.
            try {
                executor.submit(new CacheFileCleanerCallable());
            } catch (Exception e) {
                log.error("Fail to start cache cleaner", e);
        ...
        private void init() {
            ...
            try {
                ...
                for (File first : firstLevel) {
                    File[] csFiles = first.listFiles(new FileFilter() {
                        @Override
                    ...
                    for (File csFile : csFiles) {
                        ...
                        try {
                            record = getRecord(new DataIdentifier(csFile.getName()));
                            if (csFile.length() != record.length) {
                                log.warn("Cached file " + csFile.getAbsolutePath() +
        ...
        private class CacheFileCleanerCallable implements Callable<Object> {
            @Override
                ...
                try {
                    ...
                    while (cachedFilesSize.get() > lowerCacheSizeThreshold && iterator.hasNext()) {
                        ...
                        if (oldestRecord.getLastAccessTime() < scanStartTime) {
                            log.debug("Attempting to delete cache file of record: {}",
