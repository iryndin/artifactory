...
public class ChecksumsInfo implements Info {
    ...
    public ChecksumsInfo(org.artifactory.checksum.ChecksumsInfo other) {
    ...
    public void createTrustedChecksums() {
        ...
        for (ChecksumType type : types) {
            addChecksumInfo(new org.artifactory.checksum.ChecksumInfo(type, TRUSTED_FILE_MARKER, null));
        }
    }

    public boolean isIdentical(org.artifactory.checksum.ChecksumsInfo info) {
        if (this.checksums == info.getChecksums()) {
            return true;
        }
        if (this.checksums == null || info.getChecksums() == null) {
            return false;
        }
        if (this.checksums.size() != info.getChecksums().size()) {
            return false;
        }
    ...
    public boolean equals(Object o) {
        ...
        org.artifactory.checksum.ChecksumsInfo that = (org.artifactory.checksum.ChecksumsInfo) o;

        if (checksums != null ? !checksums.equals(that.getChecksums()) : that.getChecksums() != null) {
            return false;
        }
