...
BuildArch:      noarch
Requires:       %{_sbindir}/useradd, %{_sbindir}/groupadd, %{_bindir}/pkill
#Prefix:         /opt
...
%__cp -r %{extracted_standalone}/etc/* "%{buildroot}%{etc_dir}/"
...
%__cp -r %{extracted_standalone}/bin/* "%{buildroot}%{artifactory_bin_home}/bin/"
...
if [ "$1" = "2" ]; then
  ...
  SERVICE_STATUS=`%{_sysconfdir}/init.d/%{name} status`
...
%attr(775,root,root) %{artifactory_bin_home}/clilib
...
%config %{etc_dir}/artifactory.config.xml
%config %{etc_dir}/jetty.xml
