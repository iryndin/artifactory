 ...
 * </pre>
 *
 * Version 1.3.3 added support for multiple ldap settings, each might contains userDn and
 * one search pattern. If multiple authenticationPattern exist, we convert each one to
 * ldapSetting. 
 *
 * @author Yossi Shaul
 */
public class LdapListConverter implements XmlConverter {
    private final static Logger log = LoggerFactory.getLogger(LdapListConverter.class);
