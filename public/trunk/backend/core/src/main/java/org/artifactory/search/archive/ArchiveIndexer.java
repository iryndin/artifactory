...
public abstract class ArchiveIndexer {
    ...
    public static void index(JcrFile file) {
        ...
        if (archiveIndex != null) {
            try {
                node.setProperty(StorageConstants.PROP_ARTIFACTORY_ARCHIVE_ENTRY, archiveIndex, PropertyType.STRING);
                //Mark the files as indexed
                node.setProperty(StorageConstants.PROP_ARTIFACTORY_ARCHIVE_INDEXED, true);
            ...
            try {
                node.setProperty(StorageConstants.PROP_ARTIFACTORY_ARCHIVE_INDEXED, "failed");
    ...
    public static List<RepoPath> searchMarkedArchives(JcrSession session) {
        ...
        try {
            /*RowIterator rowIterator = GQL.execute(
                    ...
                    usession);*/
            String queryStr = "/jcr:root" + PathFactoryHolder.get().getAllRepoRootPath() + "//element(*, " +
                    StorageConstants.NT_ARTIFACTORY_FILE + ")[@" + StorageConstants.PROP_ARTIFACTORY_ARCHIVE_INDEXED +
                    "= false()]";
            ...
            while (nodes.hasNext()) {
                ...
                if (!path.startsWith(PathFactoryHolder.get().getAllRepoRootPath())) {
