...
public class StorageServiceImpl implements InternalStorageService {
    ...
    public FileStoreStorageSummary getFileStoreStorageSummary() {
        List<File> binariesDirs = getStorageFolders();
    ...
    private List<File> getStorageFolders() {
        Map<FileBinaryProvider, File> map = binaryStore.getBinariesDirs();
        StorageProperties.BinaryProviderType storageType = storageProperties.getBinariesStorageType();
        if (storageType.equals(StorageProperties.BinaryProviderType.fullDb)) {
            // don't filter the cache file store in full db mode
