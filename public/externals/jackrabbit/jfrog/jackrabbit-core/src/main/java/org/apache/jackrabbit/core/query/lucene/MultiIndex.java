...
/**
 * A <code>MultiIndex</code> consists of a {@link VolatileIndex} and multiple {@link PersistentIndex}es. The goal is to
 * keep most parts of the index open with index readers and write new index data to the volatile index. When the
 * volatile index reaches a certain size (see {@link SearchIndex#setMinMergeDocs(int)}) a new persistent index is
 * created with the index data from the volatile index, the same happens when the volatile index has been idle for some
 * time (see {@link SearchIndex#setVolatileIdleTime(int)}). The new persistent index is then added to the list of
 * already existing persistent indexes. Further operations on the new persistent index will however only require an
 * <code>IndexReader</code> which serves for queries but also for delete operations on the index.
 * <p/>
 * The persistent indexes are merged from time to time. The merge behaviour is configurable using the methods: {@link
 * SearchIndex#setMaxMergeDocs(int)}, {@link SearchIndex#setMergeFactor(int)} and {@link
 * SearchIndex#setMinMergeDocs(int)}. For detailed description of the configuration parameters see also the lucene
 * <code>IndexWriter</code> class.
 * <p/>
 * This class is thread-safe.
 * <p/>
 * Note on implementation: Multiple modifying threads are synchronized on a <code>MultiIndex</code> instance itself.
 * Sychronization between a modifying thread and reader threads is done using {@link #updateMonitor} and {@link
 * #updateInProgress}.
 */
public class MultiIndex {
    ...
    /**
     * List of open persistent indexes. This list may also contain an open PersistentIndex owned by the IndexMerger
     * daemon. Such an index is not registered with indexNames and <b>must not</b> be used in regular index operations
     * (delete node, etc.)!
     */
    ...
    /**
     * If not <code>null</code> points to a valid <code>IndexReader</code> that reads from all indexes, including
     * volatile and persistent indexes.
     */
     ...
     * @param excludedIDs Set&lt;NodeId> that contains uuids that should not be indexed nor further traversed.
    ...
    /**
     * Creates an initial index by traversing the node hierarchy starting at the node with <code>rootId</code>.
     *
     ...
     *
     * @param remove collection of <code>UUID</code>s that identify documents to remove
     * @param add    collection of <code>Document</code>s to add. Some of the elements in this collection may be
    ...
    /**
     * Returns <code>IndexReader</code>s for the indexes named <code>indexNames</code>. An <code>IndexListener</code> is
     * registered and notified when documents are deleted from one of the indexes in <code>indexNames</code>.
     * <p/>
     * Note: the number of <code>IndexReaders</code> returned by this method is not necessarily the same as the number
     * of index names passed. An index might have been deleted and is not reachable anymore.
     *
    ...
    /**
     * Creates a new Persistent index. The new index is not registered with this <code>MultiIndex</code>.
     *
     * @param indexName the name of the index to open, or <code>null</code> if an index with a new name should be
     *                  created.
    ...
    /**
     * Returns <code>true</code> if this multi index has an index segment with the given name. This method even returns
     * <code>true</code> if an index segments has not yet been loaded / initialized but exists on disk.
     *
     * @param indexName the name of the index segment.
     * @return <code>true</code> if it exists; otherwise <code>false</code>.
     * @throws IOException if an error occurs while checking existence of directory.
     */
    ...
    /**
     * Replaces the indexes with names <code>obsoleteIndexes</code> with <code>index</code>. Documents that must be
     * deleted in <code>index</code> can be identified with <code>Term</code>s in <code>deleted</code>.
     *
     ...
     * @param deleted         <code>Term</code>s that identify documents that must be deleted in <code>index</code>.
    ...
    /**
     * Returns an read-only <code>IndexReader</code> that spans alls indexes of this <code>MultiIndex</code>.
     *
     ...
     * Returns an read-only <code>IndexReader</code> that spans alls indexes of this <code>MultiIndex</code>.
     *
     * @param initCache when set <code>true</code> the hierarchy cache is completely initialized before this call
     *                  returns.
     * @return an <code>IndexReader</code>.
     * @throws IOException if an error occurs constructing the <code>IndexReader</code>.
     */
     ...
     * @return the index document.
     * @throws RepositoryException if an error occurs while reading from the workspace or if there is no node with
     *                             <code>id</code>.
     */
    ...
    /**
     * Returns <code>true</code> if the redo log contained entries while this index was instantiated; <code>false</code>
     * otherwise.
     *
    ...
    /**
     * Removes the <code>index</code> from the list of active sub indexes. The Index is not acutally deleted right away,
     * but postponed to the transaction commit.
     * <p/>
     * This method does not close the index, but rather expects that the index has already been closed.
     *
    ...
    /**
     * Releases the {@link #multiReader} and sets it <code>null</code>. If the reader is already <code>null</code> this
     * method does nothing. When this method returns {@link #multiReader} is guaranteed to be <code>null</code> even if
     * an exception is thrown.
     * <p/>
     * Please note that this method does not take care of any synchronization. A caller must ensure that it is the only
     * thread operating on this multi index, or that it holds the {@link #updateMonitor}.
     *
    ...
    /**
     * Enqueues unused segments for deletion in {@link #deletable}. This method does not synchronize on {@link
     * #deletable}! A caller must ensure that it is the only one acting on the {@link #deletable} map.
     *
     ...
     * @throws IOException if an error occurs while executing the action or appending the action to the redo log.
     */
    ...
    /**
     * Checks if it is needed to commit the volatile index according to {@link SearchIndex#getMaxVolatileIndexSize()}.
     *
     * @return <code>true</code> if the volatile index has been committed, <code>false</code> otherwise.
    ...
    /**
     * Commits the volatile index to a persistent index. The new persistent index is added to the list of indexes but
     * not written to disk. When this method returns a new volatile index has been created.
     *
    ...
    /**
     * Checks the duration between the last commit to this index and the current time and flushes the index (if there
     * are changes at all) if the duration (idle time) is more than {@link SearchIndex#getVolatileIdleTime()} seconds.
     */
    ...
    /**
     * Checks the indexing queue for finished text extrator jobs and updates the index accordingly if there are any new
     * ones. This method is synchronized and should only be called by the timer task that periodically checks if there
     * are documents ready in the indexing queue. A new transaction is used when documents are transfered from the
     * indexing queue to the index.
     */
     ...
     * ones.
     *
     * @param transactionPresent whether a transaction is in progress and the current {@link #getTransactionId()} should
     *                           be used. If <code>false</code> a new transaction is created when documents are
     *                           transfered from the indexing queue to the index.
     */
    ...
    public abstract static class Action {
        ...
        /**
         * Transaction identifier for internal actions like volatile index commit triggered by timer thread.
         */
         ...
         * @throws IOException if the action fails due to some I/O error in the index or some other error.
         */
        public abstract void execute(MultiIndex index) throws IOException;

        /**
         * Executes the inverse operation of this action. That is, does an undo of this action. This default
         * implementation does nothing, but returns silently.
         *
        ...
        /**
         * Returns a <code>String</code> representation of this action that can be written to the {@link RedoLog}.
         *
    ...
    private static class AddIndex extends Action {
         ...
         * @param indexName     the name of the index to add, or <code>null</code> if an index with a new name should be
         *                      created.
         */
    ...
    private static class CreateIndex extends Action {
        ...
        /**
         * Returns the index name that has been created. If this method is called before {@link #execute(MultiIndex)} it
