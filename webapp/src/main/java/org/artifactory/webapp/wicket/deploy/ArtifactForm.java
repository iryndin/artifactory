...
import org.artifactory.webapp.wicket.HomePage;
...
public class ArtifactForm extends Form {
    ...
    private final DeployArtifactPage page;
    private SubmitLink artifactSubmit;
    private LocalRepo targetRepo;
    private CheckBox deployPomCheckbox;
    private boolean deployPom;

    public ArtifactForm(final String id, DeployArtifactPage page) {
        super(id, new CompoundPropertyModel(page.getDeployableArtifact()));
        this.page = page;
        ...
        DeployableArtifact deployableArtifact = page.getDeployableArtifact();
        ...
        Link cancel = new AjaxFallbackLink("cancel") {
            public void onClick(final AjaxRequestTarget target) {
                ArtifactForm.this.page.removeUploadedFile();
                ArtifactForm.this.page.setResponsePage(HomePage.class);
            }
        };
    ...
    protected void onSubmit() {
        final DeployableArtifact deployableArtifact = page.getDeployableArtifact();
        if (!deployableArtifact.isValid()) {
            error("Artifact deployment submission attempt ignored - form not valid.");
            return;
        }
        File uploadedFile = page.getUploadedFile();
        ...
        try {
            MavenWrapper mavenWrapper = cc.getMavenWrapper();
            artifact = createArtifact(mavenWrapper, packaging);
            mavenWrapper.deploy(uploadedFile, artifact, targetRepo);
            page.getUploadForm().info("Successfully deployed: '" + artifact + "'.");
            Model model;
            //Try to create a model
            if (deployPom) {
                //This condition is kinda stupid
                model = new Model();
            ...
            if (model != null && !packaging.equals(PackagingType.pom)) {
                Artifact pomArtifact = createArtifact(mavenWrapper, PackagingType.pom);
                ...
                mavenWrapper.deploy(pomFile, pomArtifact, targetRepo);
                page.getUploadForm().info("Successfully deployed: '" + pomArtifact + "'.");
                //Delete the file
                pomFile.delete();
            }
            enable(false);
        } catch (Exception e) {
            page.getUploadForm().error(
                    "Failed to deploy artifact '" + artifact + "'. Cause: " + e.getMessage());
    ...
    private class ValidateArtifactFormBehavior extends AjaxFormComponentUpdatingBehavior {
        ...
        protected void onUpdate(AjaxRequestTarget target) {
            DeployableArtifact artifact = page.getDeployableArtifact();
    ...
    private Artifact createArtifact(MavenWrapper mavenWrapper, PackagingType packaging) {
        DeployableArtifact deployableArtifact = page.getDeployableArtifact();
        return mavenWrapper.createArtifact(
