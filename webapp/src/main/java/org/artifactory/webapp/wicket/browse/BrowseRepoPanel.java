                                ...
                                public String doInJcr(JcrSessionWrapper session)
                                    ...
                                    for (JcrFsItem item : items) {
                                        ...
                                        if (directory) {
                                            ...
                                            //Expand folders that contain only subfolders, except
                                            //for first level folders
                                            if (path.length() > 0) {
                                                List<JcrFsItem> list = folder.getItems();
                                                boolean hasOnlyDirectoryChildren = true;
                                                //Check if a folder has only subfolder children
                                                for (JcrFsItem fsItem : list) {
                                                    if (!fsItem.isDirectory()) {
                                                        hasOnlyDirectoryChildren = false;
                                                        break;
                                                    }
                                                }
                                                if (hasOnlyDirectoryChildren) {
