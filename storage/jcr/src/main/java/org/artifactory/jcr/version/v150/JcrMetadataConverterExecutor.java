...
import org.artifactory.model.xstream.fs.FolderAdditionalInfo;
import org.artifactory.model.xstream.fs.FolderInfoImpl;
import org.artifactory.model.xstream.fs.InternalFileInfo;
import org.artifactory.model.xstream.fs.InternalFolderInfo;
import org.artifactory.model.xstream.fs.ItemAdditionalInfo;
...
public class JcrMetadataConverterExecutor {
    ...
    private MetadataDefinition<FolderInfo, MutableFolderInfo> folderInfoMD;
    private MetadataDefinition<FileInfo, MutableFileInfo> fileInfoMD;
    ...
    public JcrMetadataConverterExecutor(Session session) {
        jcrSession = session;
        mdService = StorageContextHelper.get().beanForType(MetadataDefinitionService.class);
    ...
    private void convertNode(Node node) {
        try {
            boolean mixinApplied = checkMixins(node, StorageConstants.MIX_ARTIFACTORY_BASE);
            if (mixinApplied) {
    ...
    private void convertMetadataNode(Node node, Node metadataNode) throws RepositoryException {
        ...
        if (StorageConstants.NT_ARTIFACTORY_METADATA.equals(typeName)) {
            // Already done
            return;
        }
    ...
    private void convertItemInfo(Node node, boolean folder) throws RepositoryException {
        ...
        if (addInfoNode == null) {
            ...
            if (xmlData == null) {
                ...
                MutableItemInfo itemInfo;
                if (folder) {
                    itemInfo = new FolderInfoImpl(PathFactoryHolder.get().getRepoPath(node.getPath()));
                    ((InternalFolderInfo) itemInfo).setAdditionalInfo((FolderAdditionalInfo) itemAdditionalInfo);
                } else {
                    itemInfo = new FileInfoImpl(PathFactoryHolder.get().getRepoPath(node.getPath()));
                ...
                if (!folder) {
                    ...
                    ((MutableFileInfo) itemInfo).setSize(getLength(resNode));
                    ((MutableFileInfo) itemInfo).setMimeType(getMimeType(resNode));
                }
    ...
    private void convertBuildNodeFromXmlData(Node buildNode, String xmlData) throws IOException, RepositoryException {
        ...
        try {
            ...
            if (buildNode.hasProperty(StorageConstants.PROP_ARTIFACTORY_CREATED)) {
                createdDate = buildNode.getProperty(StorageConstants.PROP_ARTIFACTORY_CREATED).getDate();
            }

            Calendar lastModified = Calendar.getInstance();
            if (buildNode.hasProperty(StorageConstants.PROP_ARTIFACTORY_LAST_MODIFIED)) {
                lastModified = buildNode.getProperty(StorageConstants.PROP_ARTIFACTORY_LAST_MODIFIED).getDate();
            }

            String userId = SecurityService.USER_SYSTEM;
            if (buildNode.hasProperty(StorageConstants.PROP_ARTIFACTORY_LAST_MODIFIED_BY)) {
                userId = buildNode.getProperty(StorageConstants.PROP_ARTIFACTORY_LAST_MODIFIED_BY).getString();
            }
            ...
            newBuildNode.setProperty(StorageConstants.PROP_ARTIFACTORY_CREATED, createdDate);
            newBuildNode.setProperty(StorageConstants.PROP_ARTIFACTORY_LAST_MODIFIED, lastModified);
            newBuildNode.setProperty(StorageConstants.PROP_ARTIFACTORY_LAST_MODIFIED_BY, userId);
            saveBuildChecksumsProperty(newBuildNode, StorageConstants.PROP_BUILD_ARTIFACT_CHECKSUMS, artifactChecksums);
            saveBuildChecksumsProperty(newBuildNode, StorageConstants.PROP_BUILD_DEPENDENCY_CHECKSUMS,
                    dependencyChecksums);
    ...
    private void switchChecksumProperties(Node artConfNode) {
        ...
        for (ChecksumType type : types) {
            ...
            String actualPropName = ChecksumStorageHelper.getActualPropName(type);
    ...
    public static String getChecksumOldPropertyName(ChecksumType checksumType) {
        return StorageConstants.ARTIFACTORY_PREFIX + checksumType.alg();
    }
}