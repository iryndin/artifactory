...
public class UserGroupsDaoTest extends SecurityBaseDaoTest {
    ...
    @Test(dependsOnMethods = "testGetUsersWitchPasswordIsAboutToExpire")
    public void testDeleteUser() throws SQLException {
        assertEquals(userGroupsDao.deleteUser("u4"), 3);
    ...
    public void testRegisterIncorrectLoginAttempt() throws SQLException {
        User user = userGroupsDao.findUserById(4L);
        assertNotNull(user);
        assertEquals(user.getUserId(), 4L);
        assertEquals(user.getUsername(), "u4");
        assertTrue(userGroupsDao.registerIncorrectLoginAttempt(user));
        User readUser = userGroupsDao.findUserById(4L);
        assertNotNull(readUser);
        assertTrue(userGroupsDao.getIncorrectLoginAttempts(user.getUsername()) == 1);
    }

    @Test(dependsOnMethods = "testRegisterIncorrectLoginAttempt")
    public void testResetIncorrectLoginAttempts() throws SQLException {
        User user = userGroupsDao.findUserById(4L);
        assertNotNull(user);
        assertEquals(user.getUserId(), 4L);
        assertEquals(user.getUsername(), "u4");
        assertTrue(userGroupsDao.getIncorrectLoginAttempts(user.getUsername()) == 1);
        assertTrue(userGroupsDao.resetIncorrectLoginAttempts(user.getUsername()));
        user = userGroupsDao.findUserById(4L);
        assertNotNull(user);
        assertTrue(userGroupsDao.getIncorrectLoginAttempts(user.getUsername()) == 0);
    }
    ...
    public void testUnExpirePassword() throws SQLException {
        User user = userGroupsDao.findUserById(4L);
        assertNotNull(user);
        assertEquals(user.getUserId(), 4L);
        assertEquals(user.getUsername(), "u4");

        userGroupsDao.unExpireUserPassword(user.getUsername());
    ...
    @Test(dependsOnMethods = "testUnExpirePassword")
    ...
    public void testUnExpirePasswordsForAllUsers() throws SQLException {
        User user = userGroupsDao.findUserById(4L);
        assertNotNull(user);
        assertEquals(user.getUserId(), 4L);
        assertEquals(user.getUsername(), "u4");
    ...
    @Test(dependsOnMethods = "testUnExpirePasswordsForAllUsers")
    ...
    public void testMarkCredentialsExpired() throws SQLException {
        ...
        userGroupsDao.markCredentialsExpired(1l);
    ...
    public void testGetUsersWitchPasswordIsAboutToExpire() throws SQLException {
        userGroupsDao.unExpireUserPassword("u4");
        ...
        // since password was created today (along with user),
        // we pass artificial millisDaysToKeepPassword 1 (less than 1 day)
        DateTime now = DateTime.now();
        Map<UserInfo, Long> usersWitchPasswordIsAboutToExpire =
                userGroupsDao.getUsersWitchPasswordIsAboutToExpire(
                        now.minusDays(1).getMillis(),
                        now.getMillis(),
                        1l
                );

        assertEquals(usersWitchPasswordIsAboutToExpire.size(), 1);
        assertEquals(
                usersWitchPasswordIsAboutToExpire.keySet().toArray(new UserInfo[0])[0].getUsername(),
                user.getUsername()
        );
    }
}
