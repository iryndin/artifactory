...
public class MavenModelUtils {
    ...
    public static void readModel(InputStream is, MavenArtifactInfo da)
    ...
    private static void gatherInfoFromFile(File file, MavenArtifactInfo result) {
        ...
        if (result.hasVersion() && !result.hasClassifier()) {
            manuallyCalculateClassifier(result, fileName);
        }
    }
    ...
    /**
     * Tries to manually calculate the file's classifier through the version (if exists)
     *
     * @param result   MavenArtifactInfo object to append classifier to
     * @param fileName The uploaded file's name
     */
    private static void manuallyCalculateClassifier(MavenArtifactInfo result, String fileName) {
        String version = result.getVersion();

        //Get the version's start index
        int versionBeginIdx = fileName.lastIndexOf(version);

        //The version's end index should be the classifier's start
        int classifierBeginIdx = versionBeginIdx + version.length();

        //Get the extension's start index
        int extBeginIdx = fileName.lastIndexOf('.');

        /**
         * If:
         * -The version begin index is valid
         * -The classifier begin index is not actually the extensions start index ('.')
         * -The classifier begin index is a '-' character
         */
        if (versionBeginIdx > 0 && classifierBeginIdx < extBeginIdx &&
                fileName.charAt(classifierBeginIdx) == '-') {
            //Calculate classifier text
