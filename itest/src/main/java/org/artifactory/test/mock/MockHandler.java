...
public class MockHandler extends DefaultHandler {
    ...
    public void handle(String target, HttpServletRequest request, HttpServletResponse response, int dispatch)
        ...
        if (pathInfo.startsWith(CONFIG_PREFIX)) {
            ...
            if (!pathInfo.endsWith(CONFIG_PREFIX)) {
                ...
                String urlTestName = pathInfo.substring((pathInfo.lastIndexOf(CONFIG_PREFIX) + CONFIG_PREFIX.length()) + 1);
                ...
                if ("PUT".equalsIgnoreCase(request.getMethod())) {
                    ...
                    if (urlTestName.equals(test.name)) {
                        //Add test and send confirmation response
                        allTests.put(test.name, test);
                        response.setContentType("text/plain; charset=utf-8");
                        response.setStatus(HttpStatus.SC_OK);
                        ServletOutputStream outputStream = response.getOutputStream();
                        outputStream.write(("OK for " + test.name).getBytes("utf-8"));
                        outputStream.flush();
                    }
                    //If the test names in the url and xml do not match
                    else {
                        sendError(response, HttpStatus.SC_NOT_FOUND, "The name of the test in the URL does not match the name of the test in the XML.");
                    }
                }
                //If the config request is of GET type
                    ...
                    for (MockTest test : testCollection) {
                        if (urlTestName.equalsIgnoreCase(test.name)) {
                            testFound = true;
                    ...
                    if (!testFound) {
                        sendError(response, HttpStatus.SC_NOT_FOUND, "Cannot find requested test. Please configure it.");
                    }
                }
            ...
            //If there is no test name specified, or if the test is named 'config'
                ...
                sendError(response, HttpStatus.SC_NOT_FOUND, "Cannot configure a test named 'config'.");
                return;
            }
        }
        ...
        if (pathTest != null) {
            if (sendResponse(response, pathTest)) return;
        }
     ...
     * @throws IOException
     */
    private boolean sendResponse(HttpServletResponse response, MockPathTest pathTest) throws IOException {
        ...
        if (pathTest.urlContent != null) {
            ...
            if (pathTest.timeToTakeForData != 0) {
                ...
                for (int i = 0; i < pathTest.timeToTakeForData && byteLimit > bytesPerThreeSecs; i += 3) {
    ...
    /**
     * Searches and returns a MockPathTest via path (test.name/mock.path.test.name)
     * The replacement param is optional. If supplied, the given object will replace
     * An existing object with an identical path.
     *
     * @param pathInfo
     * @param replacement
     * @return
    ...
    private MockPathTest searchMockPathTest(String pathInfo, MockPathTest replacement) {
        Collection<MockTest> testCollection = allTests.values();
        for (MockTest test : testCollection) {
            if (pathInfo.startsWith("/" + test.name)) {
                String pathLeft = pathInfo.substring(test.name.length() + 1);
                List<MockPathTest> mockPathTests = test.paths;
